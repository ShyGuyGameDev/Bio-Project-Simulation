...
var shaayerNxtBtn;
var shaayerSkipNextBtn;
var shaayerSkipPrevBtn;
...
shaayerSkipNextBtn = new button(0, 0, 70, 40)
shaayerSkipPrevBtn = new button(0, 0, 70, 40)
...
function shaayerSetup() {
    ...
    // Position skip buttons at bottom left
    shaayerSkipPrevBtn.x = 60
    shaayerSkipPrevBtn.y = 800 - 35
    shaayerSkipNextBtn.x = 140
    shaayerSkipNextBtn.y = 800 - 35
...
function shaayerDrawSkipButtons() {
    // Only show on screens 1-8 (the interactive dragging screens)
    if (shaayerScreen < 1 || shaayerScreen > 8) return;

    push()

    // Previous button
    translate(shaayerSkipPrevBtn.x, shaayerSkipPrevBtn.y)
    rectMode(CENTER)
    fill(shaayerSkipPrevBtn.hover ? 80 : 50)
    stroke(200)
    strokeWeight(2)
    if (shaayerSkipPrevBtn.hover) {
        shaayerSkipPrevBtn.sizev = max(0.03, shaayerSkipPrevBtn.sizev);
        scale(shaayerSkipPrevBtn.size, shaayerSkipPrevBtn.size);
    } else {
        shaayerSkipPrevBtn.sizev = min(0.01, shaayerSkipPrevBtn.sizev)
        scale(shaayerSkipPrevBtn.size, shaayerSkipPrevBtn.size)
    }
    rect(0, 0, shaayerSkipPrevBtn.sizeX, shaayerSkipPrevBtn.sizeY, 5)
    ...
    shaayerSkipPrevBtn.work()
    if (shaayerSkipPrevBtn.clicked && shaayerScreen > 1) {
    ...
    push()
    // Next/Skip button
    translate(shaayerSkipNextBtn.x, shaayerSkipNextBtn.y)
    rectMode(CENTER)
    fill(shaayerSkipNextBtn.hover ? 80 : 50)
    stroke(200)
    strokeWeight(2)
    if (shaayerSkipNextBtn.hover) {
        shaayerSkipNextBtn.sizev = max(0.03, shaayerSkipNextBtn.sizev);
        scale(shaayerSkipNextBtn.size, shaayerSkipNextBtn.size);
    } else {
        shaayerSkipNextBtn.sizev = min(0.01, shaayerSkipNextBtn.sizev)
        scale(shaayerSkipNextBtn.size, shaayerSkipNextBtn.size)
    }
    rect(0, 0, shaayerSkipNextBtn.sizeX, shaayerSkipNextBtn.sizeY, 5)
    textAlign(CENTER, CENTER)
    textSize(14)
    fill("white")
    noStroke()
    ...
    shaayerSkipNextBtn.work()
    if (shaayerSkipNextBtn.clicked && shaayerScreen < 9) {
        // Skip to next screen
        shaayerScreen += 1
        objects = []
        shaayerChecks = [false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false]
